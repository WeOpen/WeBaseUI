<script lang="ts">
  ...
  interface Props extends Omit<HTMLLabelAttributes, 'children'> { name: string; label: string; value: string; selected?: string; disabled?: boolean; description?: string; }
  let { name, label, value, selected = $bindable(''), disabled = false, description = '', class: className = '', ...rest }: Props = $props();
</script>
