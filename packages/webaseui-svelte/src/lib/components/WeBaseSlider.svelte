<script lang="ts">
  ...
  interface Props extends Omit<HTMLLabelAttributes, 'children' | 'for'> { label: string; value?: number; min?: number; max?: number; step?: number; unit?: string; id?: string; }
  const uid = $props.id();
  let { label, value = $bindable(48), min = 0, max = 100, step = 1, unit = '%', id = uid, class: className = '', ...rest }: Props = $props();
...
<style>
  ...
  input:focus-visible { outline: var(--focus-ring); outline-offset: 4px; }
