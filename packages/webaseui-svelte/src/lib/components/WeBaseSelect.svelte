...
<div class={`ds-select ${className}`} bind:this={root} {...rest}>
  ...
  <div class="ds-select-menu" id={menuId} role="listbox" aria-labelledby={`${id}-label`} hidden={!open}>
...
<style>
  ...
  .ds-select-trigger[aria-invalid='true'] { border-color: var(--status-error); box-shadow: 4px 4px 0 color-mix(in srgb, var(--status-error) 14%, transparent); }
