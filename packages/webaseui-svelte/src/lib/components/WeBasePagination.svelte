<script lang="ts">
  ...
  interface Props extends Omit<HTMLAttributes<HTMLElement>, 'children'> { total?: number; page?: number; label?: string; }
  let { total = 5, page = $bindable(1), label = 'Pagination', class: className = '', ...rest }: Props = $props();
...
<nav class={`ds-pagination ${className}`} aria-label={label} {...rest}>
  <button type="button" aria-label="Previous page" disabled={page <= 1} onclick={() => (page = Math.max(1, page - 1))}><WeBaseIcon name="arrow-left" size={16} /></button>
  ...
  <button type="button" aria-label="Next page" disabled={page >= total} onclick={() => (page = Math.min(total, page + 1))}><WeBaseIcon name="arrow-right" size={16} /></button>
</nav>
