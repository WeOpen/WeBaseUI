<script lang="ts">
  ...
  const uid = $props.id();
  let { label, id = uid, placeholder = '', help = '', error = '', type = 'text', value = $bindable(''), required = false, disabled = false, class: className = '', ...rest }: Props = $props();
...
<style>
  ...
  input[aria-invalid='true'] { border-color: var(--status-error); box-shadow: 4px 4px 0 color-mix(in srgb, var(--status-error) 14%, transparent); }
